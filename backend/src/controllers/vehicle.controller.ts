...
export const assign_vehicle = async(req: AuthRequest,res: Response)=>{
    try{
        ...
        const { vehicle_id, name, registration, make, model, year, fuel_type } = req.body;

        //Validate required fields
        if(!vehicle_id || !make || !model || !year || !fuel_type){
            res.status(400).json({
                message: "Missing required fields: vehicle_id, make, model, year, fuel_type"
            });
            return;
        }
